  ...
  var TEXT = {
    ...
    reason: function (r) {
      switch (r.code) {
        ...
        case 'ngFixed': return '「' + r.a + '」と「' + r.b + '」は隣にしない組ですが、2 人とも固定席で、その席どうしが隣です。どちらかの固定席を変えてください。';
        case 'genderOver': return '「男女交互」にするには、' + gName(r.g) + 'は最大 ' + r.max + ' 人までですが、' + r.count + ' 人います。「男女交互」を外すか、席の並び（2 人机・列の数）を変えてください。';
        case 'conflict':
          if (!r.others.length) return CAT[r.cat] + 'を、この席の並びでは全部は守れません。' + (r.cat === 'ng' ? '組を減らすか、「隣」の範囲を狭くしてください。' : '条件を見直してください。');
    ...
    note: function (n) {
      switch (n.code) {
        ...
        case 'fixedSeatGone': return '「' + n.name + '」の固定席が、今の席の並びにありません（使わない席にした・列を減らした など）。固定しませんでした。';
        case 'fixedSeatTaken': return '席 No.' + n.seatNo + ' は「' + n.other + '」の固定席なので、「' + n.name + '」は固定しませんでした。';
        case 'zoneFixed': return '「' + n.name + '」は固定席があるので、前の席・後ろの席の指定は使いませんでした。';
    ...
    violation: function (v) {
      switch (v.code) {
        case 'vFixed': return '「' + v.name + '」が固定席にいません';
    ...
    ui: {
      ...
      printNeedResult: '先に「席替えする」で席を決めてください。',
